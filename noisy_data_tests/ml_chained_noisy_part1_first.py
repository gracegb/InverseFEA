...
Chain order (Part1-first):
- Model A (RF_1): Part1_E ~ X
- Model B (RF_3): Part3_E ~ X + Part1_E_pred
- Model C (RF_11): Part11_E ~ X + Part1_E_pred + Part3_E_pred
...
def chained_fold_train_predict(
    ...
    Returns:
        models: dict of trained models {"RF_1": ..., "RF_3": ..., "RF_11": ...}
    ...
    rf1.fit(X_train, y_train["Part1_E"])
    models["RF_1"] = rf1

    # B) RF_3: Part3_E ~ X + Part1_E_pred (train uses true Part1_E; test uses predicted)
    X_train_B = X_train.copy()
    X_train_B["Part1_E_pred"] = y_train["Part1_E"]  # ground-truth for training
    rf3 = RandomForestRegressor(**RF_PARAMS)
    rf3.fit(X_train_B, y_train["Part3_E"])
    models["RF_3"] = rf3

    # C) RF_11: Part11_E ~ X + Part1_E_pred + Part3_E_pred
    X_train_C = X_train.copy()
    X_train_C["Part1_E_pred"] = y_train["Part1_E"]
    X_train_C["Part3_E_pred"] = y_train["Part3_E"]
    rf11 = RandomForestRegressor(**RF_PARAMS)
    rf11.fit(X_train_C, y_train["Part11_E"])
    models["RF_11"] = rf11

    # --- Test-time predictions chained ---
    y1_hat = rf1.predict(X_test)
    X_test_B = X_test.copy()
    X_test_B["Part1_E_pred"] = y1_hat
    y3_hat = rf3.predict(X_test_B)

    X_test_C = X_test.copy()
    X_test_C["Part1_E_pred"] = y1_hat
    X_test_C["Part3_E_pred"] = y3_hat
    y11_hat = rf11.predict(X_test_C)

    y_pred_test = pd.DataFrame({
        "Part1_E": y1_hat,
        "Part3_E": y3_hat,
        "Part11_E": y11_hat,
    }, index=X_test.index)[TARGET_COLS]
...
def run_chained_cv(
    ...
    importances_sum = {"RF_1": None, "RF_3": None, "RF_11": None}
    counts = {"RF_1": 0, "RF_3": 0, "RF_11": 0}
    ...
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X), start=1):
        ...
        # --- Simple permutation importance on test split (processed scale) ---
        try:
        ...
        X_te_B = X_te.copy()
        X_te_B["Part1_E_pred"] = models["RF_1"].predict(X_te)
        try:
            r = permutation_importance(
                models["RF_3"],
                X_te_B,
        ...
        X_te_C = X_te_B.copy()
        X_te_C["Part3_E_pred"] = models["RF_3"].predict(X_te_B)
        try:
            r = permutation_importance(
                models["RF_11"],
                X_te_C,
...
def main():
    parser = argparse.ArgumentParser(description="Chained RF (Part1-first) with normalization and OLD vs NOISY comparison")
    ...
    parser.add_argument("--out", type=str, default="outputs_part1_first", help="Output directory")
