...
used for testing.  Before modeling, all feature columns are mean-centered using
statistics computed from the clean data.  A few lightweight plots are generated
to keep the workflow familiar with previous scripts.
"""
...
def mean_center_features(
    ...
    """Mean-center the feature columns using training statistics."""
...
def main() -> None:
    ...
    print("Applying mean centering based on training features...")
    X_train, X_test, feature_means = mean_center_features(X_train, X_test)
    feature_means.to_csv(OUTPUT_DIR / "feature_means.csv", header=["mean"])
